...
public class ApeOrderAddressController {
    ...
    /**
     * 兼容前端旧接口：获取收货地址列表
     * 前端当前调用的是 POST /address/getApeOrderAddressList
     */
    @PostMapping("getApeOrderAddressList")
    public Result getApeOrderAddressList() {
    ...
    public Result setDefault(@RequestParam("id") String id) {
        ApeUser user = ShiroUtils.getUserInfo();
        // 先把该用户所有地址置为非默认（first=1）
        UpdateWrapper<ApeOrderAddress> reset = new UpdateWrapper<>();
        reset.lambda()
                .set(ApeOrderAddress::getFirst, 1)
                .eq(ApeOrderAddress::getUserId, user.getId());
        apeOrderAddressService.update(reset);

        // 再把指定地址置为默认（first=0）
        ApeOrderAddress address = new ApeOrderAddress();
        address.setId(id);
        address.setFirst(0);
        boolean ok = apeOrderAddressService.updateById(address);
        return ok ? Result.success() : Result.fail(ResultCode.COMMON_DATA_OPTION_ERROR.getMessage());
    }
