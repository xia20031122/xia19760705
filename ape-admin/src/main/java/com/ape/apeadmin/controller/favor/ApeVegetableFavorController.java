...
public class ApeVegetableFavorController {
    ...
    /** 获取收藏列表（不分页） */
    @Log(name = "获取收藏列表", type = BusinessType.OTHER)
    @PostMapping("getApeVegetableFavorList")
    public Result getApeVegetableFavorList(@RequestBody(required = false) ApeVegetableFavor apeVegetableFavor) {
        ApeUser user = ShiroUtils.getUserInfo();
        QueryWrapper<ApeVegetableFavor> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda()
                .eq(ApeVegetableFavor::getUserId, user.getId())
                .eq(apeVegetableFavor != null && StringUtils.isNotBlank(apeVegetableFavor.getName()), ApeVegetableFavor::getName, apeVegetableFavor.getName())
                .eq(apeVegetableFavor != null && StringUtils.isNotBlank(apeVegetableFavor.getUnit()), ApeVegetableFavor::getUnit, apeVegetableFavor.getUnit())
                .eq(apeVegetableFavor != null && apeVegetableFavor.getPrice() != null, ApeVegetableFavor::getPrice, apeVegetableFavor.getPrice())
                .eq(apeVegetableFavor != null && StringUtils.isNotBlank(apeVegetableFavor.getVegetableId()), ApeVegetableFavor::getVegetableId, apeVegetableFavor.getVegetableId());
        // 按ID倒序，最新的在前（因为ApeVegetableFavor没有继承BaseEntity，没有createTime字段）
        queryWrapper.orderByDesc("id");
        java.util.List<ApeVegetableFavor> list = apeVegetableFavorService.list(queryWrapper);
    ...
    public Result getApeVegetableFavorPage(@RequestBody ApeVegetableFavor apeVegetableFavor) {
        ...
        Integer pageNumber = apeVegetableFavor.getPageNumber();
        Integer pageSize = apeVegetableFavor.getPageSize();
        ...
        Page<ApeVegetableFavor> page = new Page<>(pageNumber, pageSize);
        QueryWrapper<ApeVegetableFavor> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda()
